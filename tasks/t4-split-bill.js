...
exports.splitTheBill = (group) => {
  //additional validations:
  Object.values(group).forEach((item) => {
    //additional validation (undefined or null)
    if (item === undefined || item === null) {
      throw new Error('One of fields is empty...');
    }
    //additional validation (string)
    if (typeof item !== 'number') {
      throw new Error("One of the values isn't number");
    }
  });

  const totalAmount = Object.values(group).reduce(
    (acc, amount) => acc + amount
  );
  const numberOfMemebers = Object.keys(group).length;
  const averageBill = totalAmount / numberOfMemebers;

  for (let person in group) {
    group[person] = +(group[person] - averageBill).toFixed(2);
  }
  return group;
};
